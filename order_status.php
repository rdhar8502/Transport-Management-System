...
<body>
	...
	<section>
		 <?php
			...
			$sql = "SELECT pickup, drop_loc, date, weight FROM `order-db` where email='$email'";
			$result = $conn->query($sql);

			if ($result->num_rows > 0) {
				echo "<table class='table table-hover'><tr><th>Pickup</th><th>Drop</th><th>Date</th><th>Weight</th></tr>";
				// output data of each row
				while($row = $result->fetch_assoc()) {
					echo "<tr><td>".$row["pickup"]."</td><td>".$row["drop_loc"]."</td><td>".$row["date"]."</td><td>".$row["weight"]." kg</td></tr>";
				}
				echo "</table>";
			} 
			else {
